...
def add_record():
	...
	if(len(Name)>=2):
		...
		for chars in punctuation_chars:
			if(chars in Name):
				showerror("Error","Name invalid! Special Charecters not allowed!")
			else:		
				R = roll_text.get()
				try:
					Roll = int(R)
...
#btnDelete = Button(delstu,text = "Delete Record", font= ("courier",12), background = "#978897",width = 11,command = delete)
#btnDelete.place(x = 120, y = 360)

#btnBack = Button(delstu,text = "Back", font= ("courier",12),background = "#978897",width = 11,command = f8)
